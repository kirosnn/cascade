...
Cascade uses the Yoga layout engine to provide CSS Flexbox-like capabilities for responsive terminal layouts. You can create complex, dynamic interfaces that adapt to terminal size changes.
...
```typescript
import { BoxRenderable, createCliRenderer } from "@cascade/core"
