...
Cascade gives you control over terminal cleanup. Call `renderer.destroy()` when you shut down. It restores the terminal to its original state, and it frees resources.

## Why you must handle cleanup

Cascade does not automatically clean up on `process.exit` or unhandled errors. This design gives you more control over shutdown behavior:
...
Cascade listens for common exit signals and calls `destroy()` when it receives them. By default, it handles these signals:
...
- Removes the signal and process listeners that Cascade adds
