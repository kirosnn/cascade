...
The `CliRenderer` drives Cascade. It manages terminal output, handles input events, runs the rendering loop, and provides context for creating renderables.
...
```typescript
import { Box, Text } from "@cascade/core"

// Add components to the root
renderer.root.add(Box({ width: 40, height: 10, borderStyle: "rounded" }, Text({ content: "Hello, Cascade!" })))
```
...
import { DebugOverlayCorner } from "@cascade/core"
...
**Important:** Cascade does not automatically clean up on `process.exit` or unhandled errors. This design gives you control. See [Lifecycle](/docs/core-concepts/lifecycle/) for signal handling options and best practices.

## Environment variables

| Variable                    | Description                                  |
| --------------------------- | -------------------------------------------- |
| `CASCADE_USE_ALTERNATE_SCREEN` | Override alternate screen setting            |
| `CASCADE_SHOW_STATS`           | Show debug overlay at startup                |
| `CASCADE_DEBUG`                | Enable debug input capture                   |
| `CASCADE_NO_NATIVE_RENDER`     | Disable native rendering (for debugging)     |
| `CASCADE_DUMP_CAPTURES`        | Dump captured output when the renderer exits |
| `CASCADE_OVERRIDE_STDOUT`      | Override stdout stream (for debugging)       |
| `CASCADE_USE_CONSOLE`          | Enable/disable built-in console              |
