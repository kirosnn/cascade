---
title: Colors
description: Working with RGBA colors in Cascade
order: 2
---

# Colors

Cascade uses the `RGBA` class for color representation. The class stores colors as normalized float values (0.0-1.0) internally, but provides methods for working with different color formats.
...
```typescript
import { Text, Box } from "@cascade/core"
...
```typescript
import { parseColor } from "@cascade/core"
...
```typescript
import { TextRenderable, TextAttributes, RGBA } from "@cascade/core"
