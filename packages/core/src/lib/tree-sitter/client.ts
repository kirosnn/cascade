...
registerEnvVar({
  name: "CASCADE_TREE_SITTER_WORKER_PATH",
...
export class TreeSitterClient extends EventEmitter<TreeSitterClientEvents> {
  ...
  private startWorker() {
    ...
    if (env.CASCADE_TREE_SITTER_WORKER_PATH) {
      worker_path = env.CASCADE_TREE_SITTER_WORKER_PATH
    } else if (typeof CASCADE_TREE_SITTER_WORKER_PATH !== "undefined") {
      worker_path = CASCADE_TREE_SITTER_WORKER_PATH
