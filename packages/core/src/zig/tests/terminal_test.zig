...
test "setHostEnvVar applies env overrides in shared library mode" {
    ...
    try term.setHostEnvVar(testing.allocator, "CASCADE_FORCE_UNICODE", "1");
    try testing.expect(term.caps.unicode == .unicode);

    try term.setHostEnvVar(testing.allocator, "CASCADE_GRAPHICS", "0");
    try testing.expect(term.skip_graphics_query);
}
...
test "queryTerminalSend - skips OSC 66 queries when CASCADE_FORCE_EXPLICIT_WIDTH=false" {
    if (builtin.os.tag == .windows) return error.SkipZigTest;

    var env = std.process.EnvMap.init(testing.allocator);
    defer env.deinit();
    try env.put("CASCADE_FORCE_EXPLICIT_WIDTH", "false");
...
test "queryTerminalSend - sends OSC 66 queries when CASCADE_FORCE_EXPLICIT_WIDTH=true" {
    if (builtin.os.tag == .windows) return error.SkipZigTest;

    var env = std.process.EnvMap.init(testing.allocator);
    defer env.deinit();
    try env.put("CASCADE_FORCE_EXPLICIT_WIDTH", "true");
