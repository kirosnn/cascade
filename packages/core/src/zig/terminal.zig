...
fn checkEnvironmentOverrides(self: *Terminal) void {
    ...
    if (env_map.get("CASCADE_GRAPHICS")) |val| {
    ...
    if (env_map.get("CASCADE_FORCE_WCWIDTH")) |_| {
        self.caps.unicode = .wcwidth;
    }
    if (env_map.get("CASCADE_FORCE_UNICODE")) |_| {
        self.caps.unicode = .unicode;
    }
    if (env_map.get("CASCADE_FORCE_NOZWJ")) |_| {
        self.caps.unicode = .no_zwj;
    }

    if (env_map.get("CASCADE_FORCE_EXPLICIT_WIDTH")) |val| {
