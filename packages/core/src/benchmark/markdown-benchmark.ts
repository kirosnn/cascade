...
process.env.CASCADE_OVERRIDE_STDOUT = "false"
process.env.CASCADE_USE_ALTERNATE_SCREEN = "false"
...
async function runSpawnedScenarios(plans: ScenarioPlan[]): Promise<void> {
  const tempDir = await mkdtemp(path.join(tmpdir(), "cascade-markdown-bench-"))
  const scenarioLines: string[] = []
  const results: ScenarioResult[] = []
  ...
  for (const plan of plans) {
    ...
    const child = Bun.spawn([process.execPath, new URL(import.meta.url).pathname, ...args], {
      ...
      env: {
        ...process.env,
        CASCADE_OVERRIDE_STDOUT: "false",
        CASCADE_USE_ALTERNATE_SCREEN: "false",
      },
    })
