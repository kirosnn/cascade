...
registerEnvVar({
  name: "CASCADE_USE_ALTERNATE_SCREEN",
...
export class CliRenderer extends EventEmitter implements RenderContext {
  ...
  private _useAlternateScreen: boolean = env.CASCADE_USE_ALTERNATE_SCREEN
  ...
  constructor(
    ...
    this._useAlternateScreen = config.useAlternateScreen ?? env.CASCADE_USE_ALTERNATE_SCREEN
    ...
    if (env.CASCADE_OVERRIDE_STDOUT) {
    ...
    if (env.CASCADE_NO_NATIVE_RENDER) {
      this.renderNative = () => {
        if (this._splitHeight > 0) {
