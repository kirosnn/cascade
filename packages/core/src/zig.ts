...
if (!existsSync(targetLibPath)) {
  throw new Error(`cascade is not supported on the current platform: ${process.platform}-${process.arch}`)
}

registerEnvVar({
  name: "CASCADE_DEBUG_FFI",
...
function getCascadeLib(libPath?: string) {
  ...
  if (env.CASCADE_DEBUG_FFI || env.CASCADE_TRACE_FFI) {
    return {
...
function convertToDebugSymbols<T extends Record<string, any>>(symbols: T): T {
  ...
  if ((env.CASCADE_DEBUG_FFI || env.CASCADE_TRACE_FFI) && !exitHandlerRegistered) {
...
class FFIRenderLib implements RenderLib {
  private cascade: ReturnType<typeof getCascadeLib>
  ...
  constructor(libPath?: string) {
    this.cascade = getCascadeLib(libPath)
    this.setupLogging()
    this.setupEventBus()
  }
  ...
  private setLogCallback(callbackPtr: Pointer) {
    this.cascade.symbols.setLogCallback(callbackPtr)
  }
  ...
  private setEventCallback(callbackPtr: Pointer) {
    this.cascade.symbols.setEventCallback(callbackPtr)
  }

  public createRenderer(width: number, height: number, options: { testing?: boolean; remote?: boolean } = {}) {
    const testing = options.testing ?? false
    const remote = options.remote ?? false
    return this.cascade.symbols.createRenderer(width, height, testing, remote)
  }
  ...
  public setTerminalEnvVar(renderer: Pointer, key: string, value: string): boolean {
    ...
    return this.cascade.symbols.setTerminalEnvVar(renderer, keyBytes, keyBytes.length, valueBytes, valueBytes.length)
  }

  public destroyRenderer(renderer: Pointer): void {
    this.cascade.symbols.destroyRenderer(renderer)
  }

  public setUseThread(renderer: Pointer, useThread: boolean) {
    this.cascade.symbols.setUseThread(renderer, useThread)
  }

  public setBackgroundColor(renderer: Pointer, color: RGBA) {
    this.cascade.symbols.setBackgroundColor(renderer, color.buffer)
  }

  public setRenderOffset(renderer: Pointer, offset: number) {
    this.cascade.symbols.setRenderOffset(renderer, offset)
  }

  public updateStats(renderer: Pointer, time: number, fps: number, frameCallbackTime: number) {
    this.cascade.symbols.updateStats(renderer, time, fps, frameCallbackTime)
  }

  public updateMemoryStats(renderer: Pointer, heapUsed: number, heapTotal: number, arrayBuffers: number) {
    this.cascade.symbols.updateMemoryStats(renderer, heapUsed, heapTotal, arrayBuffers)
  }

  public getNextBuffer(renderer: Pointer): OptimizedBuffer {
    const bufferPtr = this.cascade.symbols.getNextBuffer(renderer)
    if (!bufferPtr) {
      throw new Error("Failed to get next buffer")
    }

    const width = this.cascade.symbols.getBufferWidth(bufferPtr)
    const height = this.cascade.symbols.getBufferHeight(bufferPtr)
  ...
  public getCurrentBuffer(renderer: Pointer): OptimizedBuffer {
    const bufferPtr = this.cascade.symbols.getCurrentBuffer(renderer)
    if (!bufferPtr) {
  ...
  public bufferGetCharPtr(buffer: Pointer): Pointer {
    const ptr = this.cascade.symbols.bufferGetCharPtr(buffer)
    if (!ptr) {
  ...
  public bufferGetFgPtr(buffer: Pointer): Pointer {
    const ptr = this.cascade.symbols.bufferGetFgPtr(buffer)
    if (!ptr) {
  ...
  public bufferGetBgPtr(buffer: Pointer): Pointer {
    const ptr = this.cascade.symbols.bufferGetBgPtr(buffer)
    if (!ptr) {
  ...
  public bufferGetAttributesPtr(buffer: Pointer): Pointer {
    const ptr = this.cascade.symbols.bufferGetAttributesPtr(buffer)
    if (!ptr) {
  ...
  public getBufferWidth(buffer: Pointer): number {
    return this.cascade.symbols.getBufferWidth(buffer)
  }

  public getBufferHeight(buffer: Pointer): number {
    return this.cascade.symbols.getBufferHeight(buffer)
  }

  public bufferClear(buffer: Pointer, color: RGBA) {
    this.cascade.symbols.bufferClear(buffer, color.buffer)
  }
  ...
  public bufferDrawText(
    ...
    this.cascade.symbols.bufferDrawText(buffer, textBytes, textLength, x, y, fg, bg, attributes ?? 0)
  }
  ...
  public bufferSetCellWithAlphaBlending(
    ...
    this.cascade.symbols.bufferSetCellWithAlphaBlending(buffer, x, y, charPtr, fg, bg, attributes ?? 0)
  }
  ...
  public bufferSetCell(
    ...
    this.cascade.symbols.bufferSetCell(buffer, x, y, charPtr, fg, bg, attributes ?? 0)
  }

  public bufferFillRect(buffer: Pointer, x: number, y: number, width: number, height: number, color: RGBA) {
    const bg = color.buffer
    this.cascade.symbols.bufferFillRect(buffer, x, y, width, height, bg)
  }
  ...
  public bufferDrawSuperSampleBuffer(
    ...
    this.cascade.symbols.bufferDrawSuperSampleBuffer(
      buffer,
      x,
      y,
      pixelDataPtr,
  ...
  public bufferDrawPackedBuffer(
    ...
    this.cascade.symbols.bufferDrawPackedBuffer(
      buffer,
      dataPtr,
      dataLen,
      posX,
  ...
  public bufferDrawGrayscaleBuffer(
    ...
    this.cascade.symbols.bufferDrawGrayscaleBuffer(
      buffer,
      posX,
      posY,
      intensitiesPtr,
  ...
  public bufferDrawGrayscaleBufferSupersampled(
    ...
    this.cascade.symbols.bufferDrawGrayscaleBufferSupersampled(
      buffer,
      posX,
      posY,
      intensitiesPtr,
  ...
  public bufferDrawGrid(
    ...
    this.cascade.symbols.bufferDrawGrid(
      buffer,
      borderChars,
      borderFg.buffer,
      borderBg.buffer,
  ...
  public bufferDrawBox(
    ...
    this.cascade.symbols.bufferDrawBox(
      buffer,
      x,
      y,
      width,
  ...
  public bufferResize(buffer: Pointer, width: number, height: number): void {
    this.cascade.symbols.bufferResize(buffer, width, height)
  }

  // Link API
  public linkAlloc(url: string): number {
    const urlBytes = this.encoder.encode(url)
    return this.cascade.symbols.linkAlloc(urlBytes, urlBytes.length)
  }

  public linkGetUrl(linkId: number, maxLen: number = 512): string {
    const outBuffer = new Uint8Array(maxLen)
    const actualLen = this.cascade.symbols.linkGetUrl(linkId, outBuffer, maxLen)
  ...
  public attributesWithLink(baseAttributes: number, linkId: number): number {
    return this.cascade.symbols.attributesWithLink(baseAttributes, linkId)
  }

  public attributesGetLinkId(attributes: number): number {
    return this.cascade.symbols.attributesGetLinkId(attributes)
  }

  public resizeRenderer(renderer: Pointer, width: number, height: number) {
    this.cascade.symbols.resizeRenderer(renderer, width, height)
  }

  public setCursorPosition(renderer: Pointer, x: number, y: number, visible: boolean) {
    this.cascade.symbols.setCursorPosition(renderer, x, y, visible)
  }

  public setCursorColor(renderer: Pointer, color: RGBA) {
    this.cascade.symbols.setCursorColor(renderer, color.buffer)
  }

  public getCursorState(renderer: Pointer): CursorState {
    const cursorBuffer = new ArrayBuffer(CursorStateStruct.size)
    this.cascade.symbols.getCursorState(renderer, ptr(cursorBuffer))
  ...
  public setCursorStyleOptions(renderer: Pointer, options: CursorStyleOptions): void {
    ...
    this.cascade.symbols.setCursorStyleOptions(renderer, ptr(buffer))
  }

  public render(renderer: Pointer, force: boolean) {
    this.cascade.symbols.render(renderer, force)
  }
  ...
  public createOptimizedBuffer(
    ...
    const bufferPtr = this.cascade.symbols.createOptimizedBuffer(
      width,
      height,
      respectAlpha,
      widthMethodCode,
  ...
  public destroyOptimizedBuffer(bufferPtr: Pointer) {
    this.cascade.symbols.destroyOptimizedBuffer(bufferPtr)
  }
  ...
  public drawFrameBuffer(
    ...
    this.cascade.symbols.drawFrameBuffer(targetBufferPtr, destX, destY, bufferPtr, srcX, srcY, srcWidth, srcHeight)
  }

  public setDebugOverlay(renderer: Pointer, enabled: boolean, corner: DebugOverlayCorner) {
    this.cascade.symbols.setDebugOverlay(renderer, enabled, corner)
  }

  public clearTerminal(renderer: Pointer) {
    this.cascade.symbols.clearTerminal(renderer)
  }

  public setTerminalTitle(renderer: Pointer, title: string) {
    const titleBytes = this.encoder.encode(title)
    this.cascade.symbols.setTerminalTitle(renderer, titleBytes, titleBytes.length)
  }

  public copyToClipboardOSC52(renderer: Pointer, target: number, payload: Uint8Array): boolean {
    return this.cascade.symbols.copyToClipboardOSC52(renderer, target, payload, payload.length)
  }

  public clearClipboardOSC52(renderer: Pointer, target: number): boolean {
    return this.cascade.symbols.clearClipboardOSC52(renderer, target)
  }

  public addToHitGrid(renderer: Pointer, x: number, y: number, width: number, height: number, id: number) {
    this.cascade.symbols.addToHitGrid(renderer, x, y, width, height, id)
  }

  public clearCurrentHitGrid(renderer: Pointer) {
    this.cascade.symbols.clearCurrentHitGrid(renderer)
  }

  public hitGridPushScissorRect(renderer: Pointer, x: number, y: number, width: number, height: number) {
    this.cascade.symbols.hitGridPushScissorRect(renderer, x, y, width, height)
  }

  public hitGridPopScissorRect(renderer: Pointer) {
    this.cascade.symbols.hitGridPopScissorRect(renderer)
  }

  public hitGridClearScissorRects(renderer: Pointer) {
    this.cascade.symbols.hitGridClearScissorRects(renderer)
  }
  ...
  public addToCurrentHitGridClipped(
    ...
    this.cascade.symbols.addToCurrentHitGridClipped(renderer, x, y, width, height, id)
  }

  public checkHit(renderer: Pointer, x: number, y: number): number {
    return this.cascade.symbols.checkHit(renderer, x, y)
  }

  public getHitGridDirty(renderer: Pointer): boolean {
    return this.cascade.symbols.getHitGridDirty(renderer)
  }

  public dumpHitGrid(renderer: Pointer): void {
    this.cascade.symbols.dumpHitGrid(renderer)
  }

  public dumpBuffers(renderer: Pointer, timestamp?: number): void {
    const ts = timestamp ?? Date.now()
    this.cascade.symbols.dumpBuffers(renderer, ts)
  }

  public dumpStdoutBuffer(renderer: Pointer, timestamp?: number): void {
    const ts = timestamp ?? Date.now()
    this.cascade.symbols.dumpStdoutBuffer(renderer, ts)
  }

  public restoreTerminalModes(renderer: Pointer): void {
    this.cascade.symbols.restoreTerminalModes(renderer)
  }

  public enableMouse(renderer: Pointer, enableMovement: boolean): void {
    this.cascade.symbols.enableMouse(renderer, enableMovement)
  }

  public disableMouse(renderer: Pointer): void {
    this.cascade.symbols.disableMouse(renderer)
  }

  public enableKittyKeyboard(renderer: Pointer, flags: number): void {
    this.cascade.symbols.enableKittyKeyboard(renderer, flags)
  }

  public disableKittyKeyboard(renderer: Pointer): void {
    this.cascade.symbols.disableKittyKeyboard(renderer)
  }

  public setKittyKeyboardFlags(renderer: Pointer, flags: number): void {
    this.cascade.symbols.setKittyKeyboardFlags(renderer, flags)
  }

  public getKittyKeyboardFlags(renderer: Pointer): number {
    return this.cascade.symbols.getKittyKeyboardFlags(renderer)
  }

  public setupTerminal(renderer: Pointer, useAlternateScreen: boolean): void {
    this.cascade.symbols.setupTerminal(renderer, useAlternateScreen)
  }

  public suspendRenderer(renderer: Pointer): void {
    this.cascade.symbols.suspendRenderer(renderer)
  }

  public resumeRenderer(renderer: Pointer): void {
    this.cascade.symbols.resumeRenderer(renderer)
  }

  public queryPixelResolution(renderer: Pointer): void {
    this.cascade.symbols.queryPixelResolution(renderer)
  }
  ...
  public writeOut(renderer: Pointer, data: string | Uint8Array): void {
    ...
    this.cascade.symbols.writeOut(renderer, ptr(bytes), bytes.length)
  }

  // TextBuffer methods
  public createTextBuffer(widthMethod: WidthMethod): TextBuffer {
    const widthMethodCode = widthMethod === "wcwidth" ? 0 : 1
    const bufferPtr = this.cascade.symbols.createTextBuffer(widthMethodCode)
    if (!bufferPtr) {
  ...
  public textBufferSetDefaultAttributes(buffer: Pointer, attributes: number | null): void {
    ...
    this.cascade.symbols.textBufferSetDefaultAttributes(buffer, attrValue)
  }

  public textBufferResetDefaults(buffer: Pointer): void {
    this.cascade.symbols.textBufferResetDefaults(buffer)
  }

  public textBufferGetTabWidth(buffer: Pointer): number {
    return this.cascade.symbols.textBufferGetTabWidth(buffer)
  }

  public textBufferSetTabWidth(buffer: Pointer, width: number): void {
    this.cascade.symbols.textBufferSetTabWidth(buffer, width)
  }

  public textBufferRegisterMemBuffer(buffer: Pointer, bytes: Uint8Array, owned: boolean = false): number {
    const result = this.cascade.symbols.textBufferRegisterMemBuffer(buffer, bytes, bytes.length, owned)
    if (result === 0xffff) {
  ...
  public textBufferReplaceMemBuffer(
    ...
    return this.cascade.symbols.textBufferReplaceMemBuffer(buffer, memId, bytes, bytes.length, owned)
  }

  public textBufferClearMemRegistry(buffer: Pointer): void {
    this.cascade.symbols.textBufferClearMemRegistry(buffer)
  }

  public textBufferSetTextFromMem(buffer: Pointer, memId: number): void {
    this.cascade.symbols.textBufferSetTextFromMem(buffer, memId)
  }

  public textBufferAppend(buffer: Pointer, bytes: Uint8Array): void {
    this.cascade.symbols.textBufferAppend(buffer, bytes, bytes.length)
  }

  public textBufferAppendFromMemId(buffer: Pointer, memId: number): void {
    this.cascade.symbols.textBufferAppendFromMemId(buffer, memId)
  }

  public textBufferLoadFile(buffer: Pointer, path: string): boolean {
    const pathBytes = this.encoder.encode(path)
    return this.cascade.symbols.textBufferLoadFile(buffer, pathBytes, pathBytes.length)
  }
  ...
  public textBufferSetStyledText(
    ...
    this.cascade.symbols.textBufferSetStyledText(buffer, ptr(chunksBuffer), chunks.length)
  }

  public textBufferGetLineCount(buffer: Pointer): number {
    return this.cascade.symbols.textBufferGetLineCount(buffer)
  }

  private textBufferGetPlainText(buffer: Pointer, outPtr: Pointer, maxLen: number): number {
    const result = this.cascade.symbols.textBufferGetPlainText(buffer, outPtr, maxLen)
  ...
  public textBufferGetTextRange(
    ...
    const actualLen = this.cascade.symbols.textBufferGetTextRange(
      buffer,
      startOffset,
      endOffset,
      ptr(outBuffer),
  ...
  public textBufferGetTextRangeByCoords(
    ...
    const actualLen = this.cascade.symbols.textBufferGetTextRangeByCoords(
      buffer,
      startRow,
      startCol,
      endRow,
  ...
  public createTextBufferView(textBuffer: Pointer): Pointer {
    const viewPtr = this.cascade.symbols.createTextBufferView(textBuffer)
    if (!viewPtr) {
  ...
  public destroyTextBufferView(view: Pointer): void {
    this.cascade.symbols.destroyTextBufferView(view)
  }
  ...
  public textBufferViewSetSelection(
    ...
    this.cascade.symbols.textBufferViewSetSelection(view, start, end, bg, fg)
  }

  public textBufferViewResetSelection(view: Pointer): void {
    this.cascade.symbols.textBufferViewResetSelection(view)
  }
  ...
  private textBufferViewGetSelectionInfo(view: Pointer): bigint {
    return this.cascade.symbols.textBufferViewGetSelectionInfo(view)
  }
  ...
  public textBufferViewSetLocalSelection(
    ...
    const bg = bgColor ? bgColor.buffer : null
    const fg = fgColor ? fgColor.buffer : null
    return this.cascade.symbols.textBufferViewSetLocalSelection(view, anchorX, anchorY, focusX, focusY, bg, fg)
  }

  public textBufferViewUpdateSelection(view: Pointer, end: number, bgColor: RGBA | null, fgColor: RGBA | null): void {
    const bg = bgColor ? bgColor.buffer : null
    const fg = fgColor ? fgColor.buffer : null
    this.cascade.symbols.textBufferViewUpdateSelection(view, end, bg, fg)
  }
  ...
  public textBufferViewUpdateLocalSelection(
    ...
    const bg = bgColor ? bgColor.buffer : null
    const fg = fgColor ? fgColor.buffer : null
    return this.cascade.symbols.textBufferViewUpdateLocalSelection(view, anchorX, anchorY, focusX, focusY, bg, fg)
  }

  public textBufferViewResetLocalSelection(view: Pointer): void {
    this.cascade.symbols.textBufferViewResetLocalSelection(view)
  }

  public textBufferViewSetWrapWidth(view: Pointer, width: number): void {
    this.cascade.symbols.textBufferViewSetWrapWidth(view, width)
  }

  public textBufferViewSetWrapMode(view: Pointer, mode: "none" | "char" | "word"): void {
    const modeValue = mode === "none" ? 0 : mode === "char" ? 1 : 2
    this.cascade.symbols.textBufferViewSetWrapMode(view, modeValue)
  }

  public textBufferViewSetViewportSize(view: Pointer, width: number, height: number): void {
    this.cascade.symbols.textBufferViewSetViewportSize(view, width, height)
  }

  public textBufferViewSetViewport(view: Pointer, x: number, y: number, width: number, height: number): void {
    this.cascade.symbols.textBufferViewSetViewport(view, x, y, width, height)
  }
  ...
  public textBufferViewGetVirtualLineCount(view: Pointer): number {
    return this.cascade.symbols.textBufferViewGetVirtualLineCount(view)
  }

  private textBufferViewGetLineInfoDirect(view: Pointer, outPtr: Pointer): void {
    this.cascade.symbols.textBufferViewGetLineInfoDirect(view, outPtr)
  }

  private textBufferViewGetLogicalLineInfoDirect(view: Pointer, outPtr: Pointer): void {
    this.cascade.symbols.textBufferViewGetLogicalLineInfoDirect(view, outPtr)
  }

  private textBufferViewGetSelectedText(view: Pointer, outPtr: Pointer, maxLen: number): number {
    const result = this.cascade.symbols.textBufferViewGetSelectedText(view, outPtr, maxLen)
    return typeof result === "bigint" ? Number(result) : result
  }

  private textBufferViewGetPlainText(view: Pointer, outPtr: Pointer, maxLen: number): number {
    const result = this.cascade.symbols.textBufferViewGetPlainText(view, outPtr, maxLen)
    return typeof result === "bigint" ? Number(result) : result
  }
  ...
  public textBufferViewSetTabIndicator(view: Pointer, indicator: number): void {
    this.cascade.symbols.textBufferViewSetTabIndicator(view, indicator)
  }

  public textBufferViewSetTabIndicatorColor(view: Pointer, color: RGBA): void {
    this.cascade.symbols.textBufferViewSetTabIndicatorColor(view, color.buffer)
  }

  public textBufferViewSetTruncate(view: Pointer, truncate: boolean): void {
    this.cascade.symbols.textBufferViewSetTruncate(view, truncate)
  }
  ...
  public textBufferViewMeasureForDimensions(
    ...
    const success = this.cascade.symbols.textBufferViewMeasureForDimensions(view, width, height, resultPtr)
    if (!success) {
      return null
    }
  ...
  public textBufferAddHighlightByCharRange(buffer: Pointer, highlight: Highlight): void {
    ...
    this.cascade.symbols.textBufferAddHighlightByCharRange(buffer, ptr(packedHighlight))
  }

  public textBufferAddHighlight(buffer: Pointer, lineIdx: number, highlight: Highlight): void {
    const packedHighlight = HighlightStruct.pack(highlight)
    this.cascade.symbols.textBufferAddHighlight(buffer, lineIdx, ptr(packedHighlight))
  }

  public textBufferRemoveHighlightsByRef(buffer: Pointer, hlRef: number): void {
    this.cascade.symbols.textBufferRemoveHighlightsByRef(buffer, hlRef)
  }

  public textBufferClearLineHighlights(buffer: Pointer, lineIdx: number): void {
    this.cascade.symbols.textBufferClearLineHighlights(buffer, lineIdx)
  }

  public textBufferClearAllHighlights(buffer: Pointer): void {
    this.cascade.symbols.textBufferClearAllHighlights(buffer)
  }

  public textBufferSetSyntaxStyle(buffer: Pointer, style: Pointer | null): void {
    this.cascade.symbols.textBufferSetSyntaxStyle(buffer, style)
  }

  public textBufferGetLineHighlights(buffer: Pointer, lineIdx: number): Array<Highlight> {
    const outCountBuf = new BigUint64Array(1)

    const nativePtr = this.cascade.symbols.textBufferGetLineHighlightsPtr(buffer, lineIdx, ptr(outCountBuf))
    if (!nativePtr) return []
    ...
    this.cascade.symbols.textBufferFreeLineHighlights(nativePtr, count)

    return results
  }

  public textBufferGetHighlightCount(buffer: Pointer): number {
    return this.cascade.symbols.textBufferGetHighlightCount(buffer)
  }

  public getArenaAllocatedBytes(): number {
    const result = this.cascade.symbols.getArenaAllocatedBytes()
    return typeof result === "bigint" ? Number(result) : result
  }

  public getBuildOptions(): BuildOptions {
    const optionsBuffer = new ArrayBuffer(BuildOptionsStruct.size)
    this.cascade.symbols.getBuildOptions(ptr(optionsBuffer))
  ...
  public getAllocatorStats(): AllocatorStats {
    ...
    this.cascade.symbols.getAllocatorStats(ptr(statsBuffer))
  ...
  public bufferDrawTextBufferView(buffer: Pointer, view: Pointer, x: number, y: number): void {
    this.cascade.symbols.bufferDrawTextBufferView(buffer, view, x, y)
  }

  public bufferDrawEditorView(buffer: Pointer, view: Pointer, x: number, y: number): void {
    this.cascade.symbols.bufferDrawEditorView(buffer, view, x, y)
  }

  // EditorView methods
  public createEditorView(editBufferPtr: Pointer, viewportWidth: number, viewportHeight: number): Pointer {
    const viewPtr = this.cascade.symbols.createEditorView(editBufferPtr, viewportWidth, viewportHeight)
    if (!viewPtr) {
  ...
  public destroyEditorView(view: Pointer): void {
    this.cascade.symbols.destroyEditorView(view)
  }

  public editorViewSetViewportSize(view: Pointer, width: number, height: number): void {
    this.cascade.symbols.editorViewSetViewportSize(view, width, height)
  }
  ...
  public editorViewSetViewport(
    ...
    this.cascade.symbols.editorViewSetViewport(view, x, y, width, height, moveCursor)
  }
  ...
  public editorViewGetViewport(view: Pointer): { offsetY: number; offsetX: number; height: number; width: number } {
    ...
    this.cascade.symbols.editorViewGetViewport(view, ptr(x), ptr(y), ptr(width), ptr(height))
  ...
  public editorViewSetScrollMargin(view: Pointer, margin: number): void {
    this.cascade.symbols.editorViewSetScrollMargin(view, margin)
  }

  public editorViewSetWrapMode(view: Pointer, mode: "none" | "char" | "word"): void {
    const modeValue = mode === "none" ? 0 : mode === "char" ? 1 : 2
    this.cascade.symbols.editorViewSetWrapMode(view, modeValue)
  }

  public editorViewGetVirtualLineCount(view: Pointer): number {
    return this.cascade.symbols.editorViewGetVirtualLineCount(view)
  }

  public editorViewGetTotalVirtualLineCount(view: Pointer): number {
    return this.cascade.symbols.editorViewGetTotalVirtualLineCount(view)
  }

  public editorViewGetTextBufferView(view: Pointer): Pointer {
    const result = this.cascade.symbols.editorViewGetTextBufferView(view)
    if (!result) {
  ...
  public editorViewGetLineInfo(view: Pointer): LineInfo {
    const outBuffer = new ArrayBuffer(LineInfoStruct.size)
    this.cascade.symbols.editorViewGetLineInfoDirect(view, ptr(outBuffer))
    const struct = LineInfoStruct.unpack(outBuffer)
    return {
      maxLineWidth: struct.maxWidth,
      lineStarts: struct.starts as number[],
  ...
  public editorViewGetLogicalLineInfo(view: Pointer): LineInfo {
    const outBuffer = new ArrayBuffer(LineInfoStruct.size)
    this.cascade.symbols.editorViewGetLogicalLineInfoDirect(view, ptr(outBuffer))
    const struct = LineInfoStruct.unpack(outBuffer)
    return {
      maxLineWidth: struct.maxWidth,
      lineStarts: struct.starts as number[],
  ...
  public createEditBuffer(widthMethod: WidthMethod): Pointer {
    const widthMethodCode = widthMethod === "wcwidth" ? 0 : 1
    const bufferPtr = this.cascade.symbols.createEditBuffer(widthMethodCode)
    if (!bufferPtr) {
  ...
  public destroyEditBuffer(buffer: Pointer): void {
    this.cascade.symbols.destroyEditBuffer(buffer)
  }

  public editBufferSetText(buffer: Pointer, textBytes: Uint8Array): void {
    this.cascade.symbols.editBufferSetText(buffer, textBytes, textBytes.length)
  }

  public editBufferSetTextFromMem(buffer: Pointer, memId: number): void {
    this.cascade.symbols.editBufferSetTextFromMem(buffer, memId)
  }

  public editBufferReplaceText(buffer: Pointer, textBytes: Uint8Array): void {
    this.cascade.symbols.editBufferReplaceText(buffer, textBytes, textBytes.length)
  }

  public editBufferReplaceTextFromMem(buffer: Pointer, memId: number): void {
    this.cascade.symbols.editBufferReplaceTextFromMem(buffer, memId)
  }

  public editBufferGetText(buffer: Pointer, maxLength: number): Uint8Array | null {
    const outBuffer = new Uint8Array(maxLength)
    const actualLen = this.cascade.symbols.editBufferGetText(buffer, ptr(outBuffer), maxLength)
    const len = typeof actualLen === "bigint" ? Number(actualLen) : actualLen
  ...
  public editBufferInsertChar(buffer: Pointer, char: string): void {
    ...
    this.cascade.symbols.editBufferInsertChar(buffer, charBytes, charBytes.length)
  }

  public editBufferInsertText(buffer: Pointer, text: string): void {
    const textBytes = this.encoder.encode(text)
    this.cascade.symbols.editBufferInsertText(buffer, textBytes, textBytes.length)
  }

  public editBufferDeleteChar(buffer: Pointer): void {
    this.cascade.symbols.editBufferDeleteChar(buffer)
  }

  public editBufferDeleteCharBackward(buffer: Pointer): void {
    this.cascade.symbols.editBufferDeleteCharBackward(buffer)
  }
  ...
  public editBufferDeleteRange(
    ...
    this.cascade.symbols.editBufferDeleteRange(buffer, startLine, startCol, endLine, endCol)
  }

  public editBufferNewLine(buffer: Pointer): void {
    this.cascade.symbols.editBufferNewLine(buffer)
  }

  public editBufferDeleteLine(buffer: Pointer): void {
    this.cascade.symbols.editBufferDeleteLine(buffer)
  }

  public editBufferMoveCursorLeft(buffer: Pointer): void {
    this.cascade.symbols.editBufferMoveCursorLeft(buffer)
  }

  public editBufferMoveCursorRight(buffer: Pointer): void {
    this.cascade.symbols.editBufferMoveCursorRight(buffer)
  }

  public editBufferMoveCursorUp(buffer: Pointer): void {
    this.cascade.symbols.editBufferMoveCursorUp(buffer)
  }

  public editBufferMoveCursorDown(buffer: Pointer): void {
    this.cascade.symbols.editBufferMoveCursorDown(buffer)
  }

  public editBufferGotoLine(buffer: Pointer, line: number): void {
    this.cascade.symbols.editBufferGotoLine(buffer, line)
  }

  public editBufferSetCursor(buffer: Pointer, line: number, byteOffset: number): void {
    this.cascade.symbols.editBufferSetCursor(buffer, line, byteOffset)
  }

  public editBufferSetCursorToLineCol(buffer: Pointer, line: number, col: number): void {
    this.cascade.symbols.editBufferSetCursorToLineCol(buffer, line, col)
  }

  public editBufferSetCursorByOffset(buffer: Pointer, offset: number): void {
    this.cascade.symbols.editBufferSetCursorByOffset(buffer, offset)
  }
  ...
  public editBufferGetCursorPosition(buffer: Pointer): LogicalCursor {
    ...
    this.cascade.symbols.editBufferGetCursorPosition(buffer, ptr(cursorBuffer))
    return LogicalCursorStruct.unpack(cursorBuffer)
  }

  public editBufferGetId(buffer: Pointer): number {
    return this.cascade.symbols.editBufferGetId(buffer)
  }

  public editBufferGetTextBuffer(buffer: Pointer): Pointer {
    const result = this.cascade.symbols.editBufferGetTextBuffer(buffer)
    if (!result) {
  ...
  public editBufferDebugLogRope(buffer: Pointer): void {
    this.cascade.symbols.editBufferDebugLogRope(buffer)
  }

  public editBufferUndo(buffer: Pointer, maxLength: number): Uint8Array | null {
    const outBuffer = new Uint8Array(maxLength)
    const actualLen = this.cascade.symbols.editBufferUndo(buffer, ptr(outBuffer), maxLength)
    const len = typeof actualLen === "bigint" ? Number(actualLen) : actualLen
    if (len === 0) return null
    return outBuffer.slice(0, len)
  }
  ...
  public editBufferRedo(buffer: Pointer, maxLength: number): Uint8Array | null {
    const outBuffer = new Uint8Array(maxLength)
    const actualLen = this.cascade.symbols.editBufferRedo(buffer, ptr(outBuffer), maxLength)
    const len = typeof actualLen === "bigint" ? Number(actualLen) : actualLen
    if (len === 0) return null
    return outBuffer.slice(0, len)
  }
  ...
  public editBufferCanUndo(buffer: Pointer): boolean {
    return this.cascade.symbols.editBufferCanUndo(buffer)
  }

  public editBufferCanRedo(buffer: Pointer): boolean {
    return this.cascade.symbols.editBufferCanRedo(buffer)
  }

  public editBufferClearHistory(buffer: Pointer): void {
    this.cascade.symbols.editBufferClearHistory(buffer)
  }

  public editBufferClear(buffer: Pointer): void {
    this.cascade.symbols.editBufferClear(buffer)
  }

  public editBufferGetNextWordBoundary(buffer: Pointer): LogicalCursor {
    const cursorBuffer = new ArrayBuffer(LogicalCursorStruct.size)
    this.cascade.symbols.editBufferGetNextWordBoundary(buffer, ptr(cursorBuffer))
    return LogicalCursorStruct.unpack(cursorBuffer)
  }

  public editBufferGetPrevWordBoundary(buffer: Pointer): LogicalCursor {
    const cursorBuffer = new ArrayBuffer(LogicalCursorStruct.size)
    this.cascade.symbols.editBufferGetPrevWordBoundary(buffer, ptr(cursorBuffer))
    return LogicalCursorStruct.unpack(cursorBuffer)
  }

  public editBufferGetEOL(buffer: Pointer): LogicalCursor {
    const cursorBuffer = new ArrayBuffer(LogicalCursorStruct.size)
    this.cascade.symbols.editBufferGetEOL(buffer, ptr(cursorBuffer))
    return LogicalCursorStruct.unpack(cursorBuffer)
  }

  public editBufferOffsetToPosition(buffer: Pointer, offset: number): LogicalCursor | null {
    const cursorBuffer = new ArrayBuffer(LogicalCursorStruct.size)
    const success = this.cascade.symbols.editBufferOffsetToPosition(buffer, offset, ptr(cursorBuffer))
    if (!success) return null
    return LogicalCursorStruct.unpack(cursorBuffer)
  }

  public editBufferPositionToOffset(buffer: Pointer, row: number, col: number): number {
    return this.cascade.symbols.editBufferPositionToOffset(buffer, row, col)
  }

  public editBufferGetLineStartOffset(buffer: Pointer, row: number): number {
    return this.cascade.symbols.editBufferGetLineStartOffset(buffer, row)
  }
  ...
  public editBufferGetTextRange(
    ...
    const outBuffer = new Uint8Array(maxLength)
    const actualLen = this.cascade.symbols.editBufferGetTextRange(
      buffer,
      startOffset,
      endOffset,
      ptr(outBuffer),
  ...
  public editBufferGetTextRangeByCoords(
    ...
    const outBuffer = new Uint8Array(maxLength)
    const actualLen = this.cascade.symbols.editBufferGetTextRangeByCoords(
      buffer,
      startRow,
      startCol,
      endRow,
  ...
  public editorViewSetSelection(
    ...
    const bg = bgColor ? bgColor.buffer : null
    const fg = fgColor ? fgColor.buffer : null
    this.cascade.symbols.editorViewSetSelection(view, start, end, bg, fg)
  }

  public editorViewResetSelection(view: Pointer): void {
    this.cascade.symbols.editorViewResetSelection(view)
  }

  public editorViewGetSelection(view: Pointer): { start: number; end: number } | null {
    const packedInfo = this.cascade.symbols.editorViewGetSelection(view)
    if (packedInfo === 0xffff_ffff_ffff_ffffn) {
      return null
    }
    const start = Number(packedInfo >> 32n)
  ...
  public editorViewSetLocalSelection(
    ...
    const bg = bgColor ? bgColor.buffer : null
    const fg = fgColor ? fgColor.buffer : null
    return this.cascade.symbols.editorViewSetLocalSelection(
      view,
      anchorX,
      anchorY,
      focusX,
  ...
  public editorViewUpdateSelection(view: Pointer, end: number, bgColor: RGBA | null, fgColor: RGBA | null): void {
    const bg = bgColor ? bgColor.buffer : null
    const fg = fgColor ? fgColor.buffer : null
    this.cascade.symbols.editorViewUpdateSelection(view, end, bg, fg)
  }
  ...
  public editorViewUpdateLocalSelection(
    ...
    const bg = bgColor ? bgColor.buffer : null
    const fg = fgColor ? fgColor.buffer : null
    return this.cascade.symbols.editorViewUpdateLocalSelection(
      view,
      anchorX,
      anchorY,
      focusX,
  ...
  public editorViewResetLocalSelection(view: Pointer): void {
    this.cascade.symbols.editorViewResetLocalSelection(view)
  }

  public editorViewGetSelectedTextBytes(view: Pointer, maxLength: number): Uint8Array | null {
    const outBuffer = new Uint8Array(maxLength)
    const actualLen = this.cascade.symbols.editorViewGetSelectedTextBytes(view, ptr(outBuffer), maxLength)
    const len = typeof actualLen === "bigint" ? Number(actualLen) : actualLen
    if (len === 0) return null
    return outBuffer.slice(0, len)
  }
  ...
  public editorViewGetCursor(view: Pointer): { row: number; col: number } {
    ...
    this.cascade.symbols.editorViewGetCursor(view, ptr(row), ptr(col))
    return { row: row[0], col: col[0] }
  }

  public editorViewGetText(view: Pointer, maxLength: number): Uint8Array | null {
    const outBuffer = new Uint8Array(maxLength)
    const actualLen = this.cascade.symbols.editorViewGetText(view, ptr(outBuffer), maxLength)
    const len = typeof actualLen === "bigint" ? Number(actualLen) : actualLen
    if (len === 0) return null
    return outBuffer.slice(0, len)
  }
  ...
  public editorViewGetVisualCursor(view: Pointer): VisualCursor {
    ...
    this.cascade.symbols.editorViewGetVisualCursor(view, ptr(cursorBuffer))
    return VisualCursorStruct.unpack(cursorBuffer)
  }

  public editorViewMoveUpVisual(view: Pointer): void {
    this.cascade.symbols.editorViewMoveUpVisual(view)
  }

  public editorViewMoveDownVisual(view: Pointer): void {
    this.cascade.symbols.editorViewMoveDownVisual(view)
  }

  public editorViewDeleteSelectedText(view: Pointer): void {
    this.cascade.symbols.editorViewDeleteSelectedText(view)
  }

  public editorViewSetCursorByOffset(view: Pointer, offset: number): void {
    this.cascade.symbols.editorViewSetCursorByOffset(view, offset)
  }

  public editorViewGetNextWordBoundary(view: Pointer): VisualCursor {
    const cursorBuffer = new ArrayBuffer(VisualCursorStruct.size)
    this.cascade.symbols.editorViewGetNextWordBoundary(view, ptr(cursorBuffer))
    return VisualCursorStruct.unpack(cursorBuffer)
  }

  public editorViewGetPrevWordBoundary(view: Pointer): VisualCursor {
    const cursorBuffer = new ArrayBuffer(VisualCursorStruct.size)
    this.cascade.symbols.editorViewGetPrevWordBoundary(view, ptr(cursorBuffer))
    return VisualCursorStruct.unpack(cursorBuffer)
  }

  public editorViewGetEOL(view: Pointer): VisualCursor {
    const cursorBuffer = new ArrayBuffer(VisualCursorStruct.size)
    this.cascade.symbols.editorViewGetEOL(view, ptr(cursorBuffer))
    return VisualCursorStruct.unpack(cursorBuffer)
  }

  public editorViewGetVisualSOL(view: Pointer): VisualCursor {
    const cursorBuffer = new ArrayBuffer(VisualCursorStruct.size)
    this.cascade.symbols.editorViewGetVisualSOL(view, ptr(cursorBuffer))
    return VisualCursorStruct.unpack(cursorBuffer)
  }

  public editorViewGetVisualEOL(view: Pointer): VisualCursor {
    const cursorBuffer = new ArrayBuffer(VisualCursorStruct.size)
    this.cascade.symbols.editorViewGetVisualEOL(view, ptr(cursorBuffer))
    return VisualCursorStruct.unpack(cursorBuffer)
  }

  public bufferPushScissorRect(buffer: Pointer, x: number, y: number, width: number, height: number): void {
    this.cascade.symbols.bufferPushScissorRect(buffer, x, y, width, height)
  }

  public bufferPopScissorRect(buffer: Pointer): void {
    this.cascade.symbols.bufferPopScissorRect(buffer)
  }

  public bufferClearScissorRects(buffer: Pointer): void {
    this.cascade.symbols.bufferClearScissorRects(buffer)
  }

  public bufferPushOpacity(buffer: Pointer, opacity: number): void {
    this.cascade.symbols.bufferPushOpacity(buffer, opacity)
  }

  public bufferPopOpacity(buffer: Pointer): void {
    this.cascade.symbols.bufferPopOpacity(buffer)
  }

  public bufferGetCurrentOpacity(buffer: Pointer): number {
    return this.cascade.symbols.bufferGetCurrentOpacity(buffer)
  }

  public bufferClearOpacity(buffer: Pointer): void {
    this.cascade.symbols.bufferClearOpacity(buffer)
  }

  public getTerminalCapabilities(renderer: Pointer) {
    const capsBuffer = new ArrayBuffer(TerminalCapabilitiesStruct.size)
    this.cascade.symbols.getTerminalCapabilities(renderer, ptr(capsBuffer))
  ...
  public processCapabilityResponse(renderer: Pointer, response: string): void {
    ...
    this.cascade.symbols.processCapabilityResponse(renderer, responseBytes, responseBytes.length)
  }
  ...
  public encodeUnicode(
    ...
    const success = this.cascade.symbols.encodeUnicode(
      textBytes,
      textBytes.length,
      ptr(outPtrBuffer),
      ptr(outLenBuffer),
  ...
  public freeUnicode(encoded: { ptr: Pointer; data: Array<{ width: number; char: number }> }): void {
    this.cascade.symbols.freeUnicode(encoded.ptr, encoded.data.length)
  }
  ...
  public bufferDrawChar(
    ...
    this.cascade.symbols.bufferDrawChar(buffer, char, x, y, fg.buffer, bg.buffer, attributes)
  }
  ...
  public registerNativeSpanFeedStream(stream: Pointer, handler: NativeSpanFeedEventHandler): void {
    ...
    this.cascade.symbols.streamSetCallback(stream, callback.ptr)
  }

  public unregisterNativeSpanFeedStream(stream: Pointer): void {
    this.cascade.symbols.streamSetCallback(stream, null)
  ...
  public createNativeSpanFeed(options?: NativeSpanFeedOptions | null): Pointer {
    ...
    const streamPtr = this.cascade.symbols.createNativeSpanFeed(optionsBuffer ? ptr(optionsBuffer) : null)
    if (!streamPtr) {
  ...
  public attachNativeSpanFeed(stream: Pointer): number {
    return this.cascade.symbols.attachNativeSpanFeed(stream)
  }

  public destroyNativeSpanFeed(stream: Pointer): void {
    this.cascade.symbols.destroyNativeSpanFeed(stream)
    this.nativeSpanFeedHandlers.delete(toPointer(stream))
  }

  public streamWrite(stream: Pointer, data: Uint8Array | string): number {
    const bytes = typeof data === "string" ? this.encoder.encode(data) : data
    return this.cascade.symbols.streamWrite(stream, ptr(bytes), bytes.length)
  }

  public streamCommit(stream: Pointer): number {
    return this.cascade.symbols.streamCommit(stream)
  }

  public streamDrainSpans(stream: Pointer, outBuffer: Uint8Array, maxSpans: number): number {
    const count = this.cascade.symbols.streamDrainSpans(stream, ptr(outBuffer), maxSpans)
    return toNumber(count)
  }

  public streamClose(stream: Pointer): number {
    return this.cascade.symbols.streamClose(stream)
  }
  ...
  public streamSetOptions(stream: Pointer, options: NativeSpanFeedOptions): number {
    ...
    return this.cascade.symbols.streamSetOptions(stream, ptr(optionsBuffer))
  }
  ...
  public streamGetStats(stream: Pointer): NativeSpanFeedStats | null {
    ...
    const status = this.cascade.symbols.streamGetStats(stream, ptr(statsBuffer))
    if (status !== 0) {
      return null
    }
  ...
  public streamReserve(stream: Pointer, minLen: number): { status: number; info: ReserveInfo | null } {
    ...
    const status = this.cascade.symbols.streamReserve(stream, minLen, ptr(reserveBuffer))
    if (status !== 0) {
  ...
  public streamCommitReserved(stream: Pointer, length: number): number {
    return this.cascade.symbols.streamCommitReserved(stream, length)
  }

  public createSyntaxStyle(): Pointer {
    const stylePtr = this.cascade.symbols.createSyntaxStyle()
    if (!stylePtr) {
  ...
  public destroySyntaxStyle(style: Pointer): void {
    this.cascade.symbols.destroySyntaxStyle(style)
  }
  ...
  public syntaxStyleRegister(
    ...
    const fgPtr = fg ? fg.buffer : null
    const bgPtr = bg ? bg.buffer : null
    return this.cascade.symbols.syntaxStyleRegister(style, nameBytes, nameBytes.length, fgPtr, bgPtr, attributes)
  }

  public syntaxStyleResolveByName(style: Pointer, name: string): number | null {
    const nameBytes = this.encoder.encode(name)
    const id = this.cascade.symbols.syntaxStyleResolveByName(style, nameBytes, nameBytes.length)
    return id === 0 ? null : id
  }

  public syntaxStyleGetStyleCount(style: Pointer): number {
    const result = this.cascade.symbols.syntaxStyleGetStyleCount(style)
    return typeof result === "bigint" ? Number(result) : result
  }
  ...
  public editorViewSetPlaceholderStyledText(
    ...
    if (nonEmptyChunks.length === 0) {
      this.cascade.symbols.editorViewSetPlaceholderStyledText(view, null, 0)
      return
    }

    const chunksBuffer = StyledChunkStruct.packList(nonEmptyChunks)
    this.cascade.symbols.editorViewSetPlaceholderStyledText(view, ptr(chunksBuffer), nonEmptyChunks.length)
  }

  public editorViewSetTabIndicator(view: Pointer, indicator: number): void {
    this.cascade.symbols.editorViewSetTabIndicator(view, indicator)
  }

  public editorViewSetTabIndicatorColor(view: Pointer, color: RGBA): void {
    this.cascade.symbols.editorViewSetTabIndicatorColor(view, color.buffer)
  }
...
let cascadeLibPath: string | undefined
let cascadeLib: RenderLib | undefined

export function setRenderLibPath(libPath: string) {
  if (cascadeLibPath !== libPath) {
    cascadeLibPath = libPath
    cascadeLib = undefined
  }
}
...
export function resolveRenderLib(): RenderLib {
  if (!cascadeLib) {
    try {
      cascadeLib = new FFIRenderLib(cascadeLibPath)
    } catch (error) {
      throw new Error(
        `Failed to initialize Cascade render library: ${error instanceof Error ? error.message : "Unknown error"}`,
      )
    }
  }
  return cascadeLib
