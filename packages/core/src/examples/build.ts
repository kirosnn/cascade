...
// Supported platforms and architectures based on bun-webgpu and cascade native binaries
const targets = [
...
for (const { platform: targetPlatform, arch: targetArch } of targets) {
  const exeName = targetPlatform === "windows" ? "cascade-examples.exe" : "cascade-examples"
  ...
  try {
    const buildResult = await Bun.build({
      ...
      compile: {
        ...
        outfile,
        execArgv: [`--user-agent=cascade-examples/${version}`, `--env-file=""`, `--`],
        windows: {},
      },
