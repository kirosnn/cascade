    ...
    case "over": {
      if (!isResizing) {
        ...
        if (direction) {
          ...
          // Note: Cascade may not support custom cursor styles yet, but we can still track the direction
        }
      }
      break
    }
...
function createDemoText(): TextNodeRenderable {
  const titleNode = TextNodeRenderable.fromString("🎨 Cascade Text Wrapping Demo", {
    fg: "#7aa2f7",
    attributes: 1, // bold
  })
  ...
  const introContNode = TextNodeRenderable.fromString(
    ". This example showcases how Cascade handles automatic text wrapping with styled content using TextNodes.",
    {
      fg: "#c0caf5",
    },
  )
...
textRenderable.add(styledText);`,
  ...
  const conclusionNode = TextNodeRenderable.fromString(
    "\n\n🚀 This demonstrates the power of Cascade's flexible text rendering system, combining rich styling with dynamic layout capabilities! ✨🎨📝",
    {
      fg: "#9ece6a",
      attributes: 1,
    },
