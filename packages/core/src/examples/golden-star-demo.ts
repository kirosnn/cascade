...
export async function run(renderer: CliRenderer): Promise<void> {
  ...
  function updateGradientBand() {
    const cascadeHeight = cascadeContainer.height
    const fiveKHeight = fiveKContainer.height

    if (cascadeHeight === 0 || fiveKHeight === 0) {
      return
    }

    const cascadeAbsY = overlayContainer.y + cascadeContainer.y
    const fiveKAbsY = overlayContainer.y + fiveKContainer.y

    const bandTop = Math.max(0, cascadeAbsY - bandPadding)
  ...
  renderer.keyInput.on("keypress", (keyEvent) => {
    if (keyEvent.name === "b") {
      ...
      if (isHellMode) {
        ...
        for (const char of cascadeChars) {
  ...
  const cascadeChars: ASCIIFontRenderable[] = []
  const cascadeContainer = new BoxRenderable(renderer, {
    id: "cascadeContainer",
    flexDirection: "row",
    justifyContent: "center",
    alignItems: "flex-end",
  ...
  overlayContainer.add(cascadeContainer)

  const cascadeText = "cascade"
  for (let i = 0; i < cascadeText.length; i++) {
    ...
    cascadeContainer.add(char)
    cascadeChars.push(char)
  }
  ...
  cascadeContainer.onSizeChange = updateGradientBand
  ...
  const charDelay = 0.1
  const charRandomOffsets: number[] = [...cascadeChars, ...fiveKChars].map(() => Math.random() * 0.1 - 0.05)
  ...
  renderer.setFrameCallback(async (deltaMs) => {
    ...
    const allChars = [...cascadeChars, ...fiveKChars]
    const jumpHeight = 3
