...
printf "${GREEN}Cascade Examples Installer${NC}\n"
echo "Installing cascade-examples binary..."
echo ""
...
ASSET_NAME="cascade-examples-v${VERSION_NO_V}-${PLATFORM}.zip"
...
if [ "$OS" != "windows" ]; then
  if [ -f "./cascade-examples" ]; then
    chmod +x ./cascade-examples
    EXEC_NAME="./cascade-examples"
  elif [ -f "./cascade-examples.exe" ]; then
    EXEC_NAME="./cascade-examples.exe"
  else
...
printf "${GREEN}✓ Cascade Examples installed successfully!${NC}\n"
echo ""
printf "${BLUE}To run the examples, execute:${NC}\n"
if [ "$OS" = "windows" ]; then
  printf "  ${GREEN}.\\\\cascade-examples.exe${NC}\n"
else
  printf "  ${GREEN}./cascade-examples${NC}\n"
fi
echo ""
