...
test("links with conceal mode", async () => {
  const markdown = `Check out [Cascade](https://github.com/sst/cascade) for more.`

  expect(await renderMarkdown(markdown)).toMatchInlineSnapshot(`
    "
    Check out Cascade (https://github.com/sst/cascade) for more."
  `)
})
...
test("links with conceal=false", async () => {
  ...
  expect(await renderMarkdown(markdown, false)).toMatchInlineSnapshot(`
    "
    Check out [Cascade](https://github.com/sst/cascade) for
    more."
  `)
})
...
Welcome to **Cascade**, a terminal UI library.
...
test("theme switching (syntaxStyle change)", async () => {
  ...
  const content = `# Cascade Markdown Demo
...
| Link | \`[text](url)\` | [Cascade](https://github.com) |
...
`
  ...
  const headingSpan1 = findSpanContaining(frame1, "Cascade Markdown Demo")
  ...
  const headingSpan2 = findSpanContaining(frame2, "Cascade Markdown Demo")
